...
export default {
  ...
  tip_infection_intensity:
    '您是否想選擇強傳染(強互惠)的協議？ 當您選擇強傳染(強互惠)許可證時，任何使用、修改或分發您的程式碼的人都必須遵循相同的授權要求。 這意味著他們必須提供原始程式碼，並將其程式碼以相同的許可證發布。 這樣，您的程式碼的開放性將被保護，並且任何人都可以獲得您的程式碼的原始程式碼，以便進行學習、改進和共享。 強傳染性許可證確保了對整個專案的開放性和共享性，促進了開源社群的合作和創新。 但是，選擇強傳染性許可證可能會對某些開發者或組織造成限制。',
