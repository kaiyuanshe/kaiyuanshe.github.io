export default {
  self_developed: '自研',
  open_source: '開源',
  project: '專案',
  our_principles: '我們的原則',
  contribution: '貢獻',
  consensus: '共識',
  co_governance: '共治',
  open_source_governance: '開源治理',
  international_bridge: '國際橋樑',
  community_development: '社區發展',
  open_source_project: '開源項目',
  our_mission: '我們的使命',
  latest_news: '最新動態',
  slogan: '身體力行地踐行開源，咱們華人有力量！',
  activity_map: '活動地圖',
  open_source_library: '開源文庫',
  full_member: '正式成員',
  organization: '組織機構',
  open_source_map: '開源地圖',
  partner: '合作夥伴',
  open_source_community: '開源社',
  no_more: '沒有更多',
  load_more: '加載更多……',
  organization_member: '組織成員',
  council: '理事會',
  organization_of_open_source_society: '開源社組織機構',
  become_volunteer: '成為志願者',
  china_open_source_map: '中國開源地圖',
  panorama: '全景圖',
  join_the_open_source_map: '+ 加入開源地圖',
  map: '地圖',
  chart: '圖表',
  related_articles: '相關文章',
  no_data: '暫無數據',
  functions: 'functions',
  executive_committee: '執行委員會',
  project_committee: '項目委員會',
  consultant_committee: '顧問委員會',
  legal_advisory_committee: '法律諮詢委員會',
  unpublished: '未公開',
  unclassified: '未分組',
  community_city_ranking: '社區城市排行',
  community_field_ranking: '社區領域排行',
  community_founding_chronology: '社區創始年表',
  community_type_distribution: '社區類型分佈',
  panorama_of_china_open_source_community: '中國開源社區全景圖',
  search_results: '搜索結果',
  article: '文章',
  member: '成員',
  department: '部門',
  organization_short: '組織',
  reset: '重置',
  altogether: '共',
  companies: '家',
  filter: '篩選',
  wonderful_activity: '精彩活動',
  member_x: '成員X',
  activity: '活動',
  strategic_sponsorship: '戰略贊助',
  platinum_sponsorship: '白金贊助',
  gold_sponsorship: '金牌贊助',
  silver_sponsorship: '銀牌贊助',
  bronze_sponsorship: '銅牌贊助',
  highlight_sponsorship: '亮點贊助',
  member_sponsorship: '成員贊助',
  international_lecturer_travel_sponsorship: '國際講師差旅贊助',
  register_platform_partner: '報名平台夥伴',
  live_video_partner: '視頻直播夥伴',
  directed_by: '指導單位',
  conference_cooperation_sponsorship: '大會合作單位',
  community_partner: '社區夥伴',
  media_partner: '媒體夥伴',
} as const;
